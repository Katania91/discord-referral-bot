...
Self-service **referral / invite system** for Discord servers:
...
## TL;DR – What this bot does (in plain English)
...
- show a **leaderboard** and give **rewards** at 5 / 15 confirmed referrals (configurable)
...
> You need to be able to run a Node.js bot yourself (this is not a hosted bot).
...
- Either:
  - **SQLite** (default, file-based, no setup) or  
...
# Recommended for testing (register commands only in your guild)
GUILD_ID=your-guild-id
...
REWARD_TIER1=5             # optional: first reward threshold (confirmed referrals)
REWARD_TIER2=15            # optional: second reward threshold (confirmed referrals)

# MySQL (only if DB_ENGINE=mysql)
...
* **MySQL/MariaDB**: apply [data/schema.sql](data/schema.sql), set `DB_ENGINE=mysql` and `MYSQL_*`.
...
* Default reward tiers are **5** and **15** confirmed referrals, configurable via `REWARD_TIER1` and `REWARD_TIER2` in your `.env`.
* You can:
...
* Minimum **account age** check (configurable in config/database).
